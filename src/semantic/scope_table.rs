...
impl Analyzer {
  ...
  pub fn add_scope(&mut self, block_id: BlockId) -> Result<(), Error> {
    match self.scope_table.insert(block_id, Rc::new(BlockScope::new())) {
      Some(_) => Err(Error::InternalError(format!(
        "failed to new scope add to block table. block id conflict: {block_id}"
      ))),
      None => Ok(())
    }
  }
  ...
  pub fn get_parent_scope(&self, block_id: &BlockId) -> Result<Rc<BlockScope>, Error> {
    self.scope_table.get(block_id).ok_or(
      Error::InternalError(format!("can't find parent block for this block {}", block_id))
    ).cloned()
  }

  /// 获取给定 BlockId 的 &mut BlockScope
  pub fn get_scope_by_id(&mut self, block_id: &BlockId) -> Result<Rc<BlockScope>, Error> {
    self.scope_table.get_mut(block_id).ok_or(
      Error::InternalError(format!("can't find the scope of this block: {}", block_id))
    ).cloned()
  }
}
...
pub struct Blocks {
  ...
  block_table: HashMap<BlockId, Rc<RefCell<Block>>>,
}
...
impl Blocks {
  ...
  pub fn add_block(&mut self, block: Rc<RefCell<Block>>) -> Result<(), Error> {
    let k = block.borrow().block_id;
  ...
  pub fn get_parent_block(&self, block_id: &BlockId) -> Result<&Rc<RefCell<Block>>, Error> {
  ...
  pub fn get_block(&self, block_id: &BlockId) -> Result<&Rc<RefCell<Block>>, Error> {
    let err: String = format!("can't find current block");
    self.block_table.get(block_id).ok_or(Error::InternalError(err))
  }

  /// 获取整个 block_table，用于交给解释器执行
  pub fn get_block_table(&self) -> &HashMap<BlockId, Rc<RefCell<Block>>> {
    &self.block_table
  }
}
...
impl Analyzer {
  ...
  pub fn func_block_check(&mut self, func_def: Rc<FuncDef>) -> Result<(), Error> {
    ...
    for param in &func_def.func_fparams {
      ...
      scope.decl(ident, false, rval.clone())?;  /* 函数没有父级 Block，无需检查上层 */
    }
